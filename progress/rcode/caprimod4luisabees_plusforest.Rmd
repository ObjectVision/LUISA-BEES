...
## Create demand files
```{r}
rawforest <- read_excel("E:\\data\\globiomg4m\\IIASA_Reference2016_2070_forest_area_G4M_iso.xlsx") 
longforest <-rawforest %>%
  gather(year,forestarea,as.character(2000):as.character(2070)) %>%
  arrange(X__1) 
longforestnoblcf<-longforest[-grep("areaused", longforest$X__2),]
forestLTS<-ddply(longforestnoblcf,c("X__1", "year"), summarise, forestarea=sum(forestarea)) 
